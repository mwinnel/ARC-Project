...
repeat {
  ...
  for ( j in 1:n ) {
    ...
    if ( UPDATE[j] ) {
      ...
      ## need this to change format of dates collected - currently stored as %d/%m/%Y - server needs as %Y/%m/%d
      
 #     dataset.mix[[j]]$Date <- changeDatefmt(dataset.mix[[j]], "%d/%m/%Y","%Y/%m/%d")
      
      ## write table to flat data files
  ...
  if(updatecounter>=11){
    ...
    if( LIVESTREAM ) {
      ...
      tryCatch({ ftpUpload(tgzName, paste("ftp://192.168.30.11/4/",tgzName,sep=""))}, condition=function(ex) {
        a <- print(ex)
